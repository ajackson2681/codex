...
/** Sentinel for optional pins (RW, EN2). Pass instead of a GPIO number. */
constexpr int LCD_PIN_NC = 0xFFu;
...
protected:
	...
	inline void delayPerHome(void) { if((_rw_pin == 255) && (userFunc == nullptr) ) delayMicroseconds(2900);}
...
private:
    // arduino wrapper functions (These just make it so the core implementation
    // doesn't have to change for pico SDK. For example, digitalWrite() is used
    // for arduino compatability, but in the pico SDK we use gpio_put() instead. 
    // These functions are implemented in LiquidCrystal.cpp and are only used
    // when not using pico SDK
