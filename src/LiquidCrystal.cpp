...
LiquidCrystal::LiquidCrystal(uint8_t rs, uint8_t rw, uint8_t enable, uint8_t en2,
							 uint8_t d0, uint8_t d1, uint8_t d2, uint8_t d3, void userBusy(int8_t chip))    //4x40  4 data pins 2 enable pins
{
...
void LiquidCrystal::send(uint8_t value, uint8_t mode) {
		...
		if(_rw_pin == 255) {
			...
			do {
				...
				delayMicroseconds(1); // clock pulse needs > 450 ns
...
int LiquidCrystal::digitalRead(int pin) {
    return gpio_get(pin);
}

void LiquidCrystal::pinMode(int pin, PinMode mode) {
    switch (mode) {
        case INPUT:
            gpio_init(pin);
